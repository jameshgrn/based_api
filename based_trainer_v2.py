...
from data_format_v2 import generate_data_v2

optuna.logging.set_verbosity(optuna.logging.WARNING)

FEATURES = ['log_Q', 'log_w', 'log_S']
MODEL_PATH = 'based_model_v2.ubj'
VALIDATION_IMG = 'img/BASED_validation_v2.png'
DEVICE = 'cpu'
...
def main():
    df = pd.read_csv('data/based_input_data_v2.csv')
    ...
    print(f"\nTest set metrics (original space)  n={len(y_test)}:")
    print(f"  MAE:  {mae:.3f} m")
    print(f"  RMSE: {rmse:.3f} m")
    print(f"  R2:   {r2:.4f}")
    print(f"  MAPE: {mape:.1f}%")

    # compare against v1 holdout metrics for reference
    print("\n  v1 reference: MAE 0.276m  RMSE 0.940m  R2 0.8447  MAPE 15.6%")
    ...
    ax.set_title(f'BASED v2 Validation | n = {len(y_test)}')
    ax.legend()
    plt.tight_layout()
